...
func (w *SessionFilesystemWorker) buildSessionCommitStructure(session *models.Session) (string, error) {
	...
	log.Infof("\t[S:%s] Trying to build session commit structure in folder %s", session.UUID, session.Application.Folder)
	...
	if err != nil {
		log.Errorf("\t[S:%s] Error while providing authentication %s", session.UUID, err.Error())
	...
	if _, err := os.Stat(sessionCommitFolder); os.IsNotExist(err) {
		...
		log.Infof("\t[S:%s] Cloning from remote %s into %s", session.UUID, session.Application.Remote, sessionCommitFolder)
		...
		if err != nil {
			...
			log.Errorf("\t[S:%s] Error while cloning: %s", session.UUID, err.Error())
			return "", err
		}
	}
	...
	if err != nil {
		...
		log.Errorf("\t[S:%s] Error while using existing repository %s", session.UUID, err.Error())
		return "", err
	}
	...
	if err != nil {
		...
		log.Errorf("\t[S:%s] Error while fetching from remote: %s", session.UUID, err.Error())
		return "", err
	}
	...
	if err != nil {
		...
		log.Infof("\t[S:%s] Error while retrieving worketree: %s", session.UUID, err.Error())
		return "", err
	}

	session.LogInfo("Performing an hard reset to the selected commit")
	log.Infof("\t[S:%s] Performing an hard reset to the selected commit", session.UUID)
	...
	if err != nil {
		...
		log.Infof("\t[S:%s] Error while performing hard reset: %s", session.UUID, err.Error())
		return "", err
	}
